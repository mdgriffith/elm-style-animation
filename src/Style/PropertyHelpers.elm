...
type alias Dynamic =
    Property Physics DynamicColor


type alias Static =
    Property Float ElmColor.Color


type alias Retarget =
    Property (Float -> Float) (ElmColor.Color -> ElmColor.Color)
...
renderAttr styles =
    let
        toAttr prop =
            case prop of
                X a ->
                    Just <| Svg.x (toString a)

                Y a ->
                    Just <| Svg.y (toString a)

                Cx a ->
                    Just <| Svg.cx (toString a)

                Cy a ->
                    Just <| Svg.cy (toString a)

                R a ->
                    Just <| Svg.r (toString a)

                Rx a ->
                    Just <| Svg.rx (toString a)

                Ry a ->
                    Just <| Svg.ry (toString a)

                D commands ->
                    Just <| Svg.d <| renderPath commands

                Points points ->
                    Just <| Svg.points <| renderPoints points

                Width a _ ->
                    Just <| Svg.width (toString a)

                Height a _ ->
                    Just <| Svg.height (toString a)

                Fill color ->
                    Just <| Svg.fill (renderColor color)

                Stroke color ->
                    Just <| Svg.stroke (renderColor color)
...
renderPoints points =
    String.concat <| List.intersperse " " <| List.map (\pair -> toString (fst pair) ++ "," ++ toString (snd pair)) points
...
toStatic prop =
    map (\phys -> phys.physical.position) toStaticColor prop
...
map fn colorFn prop =
    case prop of
        ...
        D cmds ->
            D (List.map (mapCmd fn) cmds)

        Points a ->
            Points <| List.map (\( x, y ) -> ( fn x, fn y )) a

        Fill color ->
            Fill <| colorFn color

        Stroke color ->
            Stroke <| colorFn color


mapPoints fn points =
    List.map (\( x, y ) -> ( fn x, fn y )) points
...
mapCmd fn cmd =
    case cmd of
        ...
        Curve points ->
            Curve (mapPoints fn points)

        CurveTo points ->
            CurveTo (mapPoints fn points)

        Quadratic points ->
            Quadratic (mapPoints fn points)

        QuadraticTo points ->
            QuadraticTo (mapPoints fn points)

        SmoothQuadratic points ->
            SmoothQuadratic (mapPoints fn points)

        SmoothQuadraticTo points ->
            SmoothQuadraticTo (mapPoints fn points)

        Smooth points ->
            Smooth (mapPoints fn points)

        SmoothTo points ->
            SmoothTo (mapPoints fn points)

        Arc rx ry x y ->
            Arc (fn rx) (fn ry) (fn x) (fn y)

        ArcTo rx ry x y ->
            ArcTo (fn rx) (fn ry) (fn x) (fn y)

        LargeArc rx ry x y ->
            LargeArc (fn rx) (fn ry) (fn x) (fn y)

        LargeArcTo rx ry x y ->
            LargeArcTo (fn rx) (fn ry) (fn x) (fn y)
        ...
        Points pts1 ->
            case matchTo of
                Points pts2 ->
                    let
                        diff =
                            List.length pts2 - List.length pts1

                        maybeLast =
                            List.head <| List.drop (List.length pts1 - 1) pts1
                    in
                        if diff > 0 then
                            case maybeLast of
                                Nothing ->
...
is pred prop =
    case prop of
        ...
        D a ->
            List.all (isCmd pred) a

        Points a ->
            List.all (\( x, y ) -> pred x && pred y) a
...
apply retarget prop =
    case retarget of
        ...
        Points a ->
            case prop of
                Points b ->
                    Points <| List.map2 (\( x1, y1 ) ( x2, y2 ) -> ( x1 x2, y1 y2 )) a b
...
applyCmd cmd cmd2 =
    case cmd of
        ...
        Curve points ->
            case cmd2 of
                Curve points2 ->
                    Curve (List.map2 (\( x, y ) ( x1, y1 ) -> ( x x1, y y1 )) points points2)
        ...
        CurveTo points ->
            case cmd2 of
                Curve points2 ->
                    CurveTo (List.map2 (\( x, y ) ( x1, y1 ) -> ( x x1, y y1 )) points points2)
        ...
        Quadratic points ->
            case cmd2 of
                Quadratic points2 ->
                    Quadratic (List.map2 (\( x, y ) ( x1, y1 ) -> ( x x1, y y1 )) points points2)
        ...
        QuadraticTo points ->
            case cmd2 of
                QuadraticTo points2 ->
                    QuadraticTo (List.map2 (\( x, y ) ( x1, y1 ) -> ( x x1, y y1 )) points points2)
        ...
        SmoothQuadratic points ->
            case cmd2 of
                SmoothQuadratic points2 ->
                    SmoothQuadratic (List.map2 (\( x, y ) ( x1, y1 ) -> ( x x1, y y1 )) points points2)
        ...
        SmoothQuadraticTo points ->
            case cmd2 of
                SmoothQuadraticTo points2 ->
                    SmoothQuadraticTo (List.map2 (\( x, y ) ( x1, y1 ) -> ( x x1, y y1 )) points points2)
        ...
        Smooth points ->
            case cmd2 of
                Smooth points2 ->
                    Smooth (List.map2 (\( x, y ) ( x1, y1 ) -> ( x x1, y y1 )) points points2)
        ...
        SmoothTo points ->
            case cmd2 of
                SmoothTo points2 ->
                    SmoothTo (List.map2 (\( x, y ) ( x1, y1 ) -> ( x x1, y y1 )) points points2)
        ...
        LargeArcTo rx ry x y ->
            case cmd2 of
                LargeArcTo rx' ry' x' y' ->
                    LargeArcTo (rx rx') (ry ry') (x x') (y y')
...
map2 fn colorFn prev prop =
    case prev of
        ...
        Points a ->
            case prop of
                Points b ->
                    Points <| List.map2 (\( x1, y1 ) ( x2, y2 ) -> ( fn x1 x2, fn y1 y2 )) a b
...
map2Points fn points points2 =
    List.map2 (\( x, y ) ( x1, y1 ) -> ( fn x x1, fn y y1 )) points points2


map3Points fn points points2 points3 =
    List.map3 (\( x, y ) ( x1, y1 ) ( x2, y2 ) -> ( fn x x1 x2, fn y y1 y2 )) points points2 points3
...
map2Cmd fn cmd cmd2 =
    case cmd of
        ...
        Curve points ->
            case cmd2 of
                Curve points2 ->
                    Curve (map2Points fn points points2)
        ...
        CurveTo points ->
            case cmd2 of
                Curve points2 ->
                    CurveTo (map2Points fn points points2)
        ...
        Quadratic points ->
            case cmd2 of
                Quadratic points2 ->
                    Quadratic (map2Points fn points points2)
        ...
        QuadraticTo points ->
            case cmd2 of
                QuadraticTo points2 ->
                    QuadraticTo (map2Points fn points points2)
        ...
        SmoothQuadratic points ->
            case cmd2 of
                SmoothQuadratic points2 ->
                    SmoothQuadratic (map2Points fn points points2)
        ...
        SmoothQuadraticTo points ->
            case cmd2 of
                SmoothQuadraticTo points2 ->
                    SmoothQuadraticTo (map2Points fn points points2)
        ...
        Smooth points ->
            case cmd2 of
                Smooth points2 ->
                    Smooth (map2Points fn points points2)
        ...
        SmoothTo points ->
            case cmd2 of
                SmoothTo points2 ->
                    SmoothTo (map2Points fn points points2)
        ...
        Arc rx ry x y ->
            case cmd2 of
                Arc rx' ry' x' y' ->
                    Arc (fn rx rx') (fn ry ry') (fn x x') (fn y y')
        ...
        ArcTo rx ry x y ->
            case cmd2 of
                ArcTo rx' ry' x' y' ->
                    ArcTo (fn rx rx') (fn ry ry') (fn x x') (fn y y')
        ...
        LargeArc rx ry x y ->
            case cmd2 of
                LargeArc rx' ry' x' y' ->
                    LargeArc (fn rx rx') (fn ry ry') (fn x x') (fn y y')
        ...
        LargeArcTo rx ry x y ->
            case cmd2 of
                LargeArcTo rx' ry' x' y' ->
                    LargeArcTo (fn rx rx') (fn ry ry') (fn x x') (fn y y')
...
updateOver fn target prev prop =
    map3
        ...
        (\targetColor prevColor currentDColor ->
            let
                t =
                    ElmColor.toRgb targetColor

                p =
                    ElmColor.toRgb prevColor
            in
                case currentDColor of
                    RGBA r1 g1 b1 a1 ->
                        RGBA
